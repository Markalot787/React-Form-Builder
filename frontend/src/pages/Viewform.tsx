// REACT-FORM-BUILDER/frontend/src/pages/Viewform.tsx
...
type FormData = {
	...
	fields: Array<{
		label: string;
		type: string;
		options: Array<string>; // Use 'options' instead of 'opts'
	}>;
	submissions: Array<{
		name: string;
		responses: Array<any>;
					...
					<Button
						onClick={() => {
							navigator.clipboard.writeText(
								`http://localhost:3000/React-Form-Builder/#/form/${formId}`
							);
						}}
					>
						<LinkSharp />
									...
									{field.type === 'chk' ? (
										<>
											{field.options.map((option: string, j: number) => {
												const isSelected =
													data.submissions[parseInt(value)].responses[i][j];
												return (
													<Chip
														label={option}
														color={isSelected === true ? 'success' : 'error'}
														size={'small'}
														style={{
															width: 'fit-content',
															margin: '5px auto',
